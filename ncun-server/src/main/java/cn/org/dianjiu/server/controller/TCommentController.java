...
public class TCommentController {
    ...
    public RespVO<PageResp> listByPage(@RequestBody PageReq<TCommentReq> pageReq) {
        PageResp<List<TCommentResp>> pageVO = new PageResp<>();
        RespVO<PageResp> result = new RespVO<>();
        PageInfo<TCommentResp> pages = tCommentService.listByPage(pageReq);
    ...
    public RespVO<TCommentResp> insertBatch(@RequestBody List<TCommentReq> list) {
        RespVO<TCommentResp> result = new RespVO<>();
        int insert = tCommentService.insertBatch(list);
        if (insert < 1) {
            result.setCode("400");
            result.setMsg("新增数据失败！");
            return result;
